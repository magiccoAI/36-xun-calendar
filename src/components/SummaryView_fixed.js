        ...
        if (xunData.every(d => !d.hasData)) {
            ...
            xunData.forEach(data => {
                if (data.hasData) {
                    ...
                    if (data.metrics) {
                        if (data.metrics.sleep) { 
                            stats.sleep.total += data.metrics.sleep; 
                            stats.sleep.count++; 
                        }
                        if (data.metrics.exercise) { 
                            stats.exercise.total += data.metrics.exercise; 
                            stats.exercise.count++; 
                        }
                        if (data.metrics.reading) { 
                            stats.reading.total += data.metrics.reading; 
                            stats.reading.count++; 
                        }
                    ...
                    // 新的睡眠数据结构
                    if (data.sleepData && data.sleepData.duration) {
                        stats.sleep.total += data.sleepData.duration;
                        stats.sleep.count++;
                        stats.sleepData.push(data.sleepData.duration);
                    } else if (data.metrics?.sleep) {
                        // 兼容旧数据
                        stats.sleep.total += data.metrics.sleep;
                        stats.sleep.count++;
                        stats.sleepData.push(data.metrics.sleep);
                    } else {
                ...
                new Chart(moodChartEl, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            ...
                            hoverBackgroundColor: ['#DC2626', '#EA580C', '#CA8A04', '#2563EB', '#16A34A'],
                            borderWidth: 2,
                            borderColor: '#ffffff'
                        }]
                    },
